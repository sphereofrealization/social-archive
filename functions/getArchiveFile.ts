...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (checkBudget('friends')) {
      ...
      for (const f of filesToRead) {
        ...
        try {
          ...
          if (f.ext === 'html') {
            ...
            // Extract friend entries from HTML table rows
            $('tr').each((_, row) => {
              const tds = $(row).find('td');
              if (tds.length > 0) {
                const name = $(tds[0]).text().trim();
                if (name && name.length > 1 && name.length < 100 && !name.match(/^[\d\s]*$/)) {
                  friendCount++;
                }
              }
            });
            ...
            $('li').each((_, el) => {
              const text = $(el).text().trim();
              if (text && text.length > 1 && text.length < 100 && !text.match(/^[\d\s]*$/) && !text.includes('Generated by')) {
                if (!result.friends.find(f => f.name === text)) {
      ...
      result.debug.friendCount = friendCount;
    }
    ...
    if (checkBudget('comments')) {
      ...
      for (const f of filesToRead) {
        ...
        try {
          ...
          if (f.ext === 'html') {
            const $ = cheerio.load(content);
            // Remove metadata sections first
            $('style, script, noscript, head, meta').remove();

            // Look for actual comment divs/paragraphs that aren't headers
            const texts = new Set();
            $('div, p').each((_, el) => {
              const text = $(el).text().trim();
              // Filter out metadata: if it contains these markers, it's likely a header
              if (text.length > 10 && text.length < 1000 &&
                  !text.includes('Generated by') &&
                  !text.includes('Contains data you requested') &&
                  !text.includes('your_facebook_activity') &&
                  !text.match(/^(Comments you|Your (posts|friends|messages))/) &&
                  !text.match(/^\d+\s*(day|hour|minute|second|week|month|year)/) &&
                  text.match(/[a-zA-Z]{5,}/)) {
                texts.add(text);
                commentCount++;
              }
      ...
      result.debug.commentCount = commentCount;
    }
    ...
    if (checkBudget('posts')) {
      ...
      for (const f of filesToRead) {
        ...
        try {
          ...
          if (f.ext === 'html') {
            const $ = cheerio.load(content);
            // Facebook archive structure: each post is wrapped in a div with class containing 'post'
            // Look for divs that have actual content (not headers/footers)
            const $divs = $('div');
            let foundPostContent = false;

            $divs.each((_, div) => {
              const text = $(div).text().trim();
              // Look for actual post content - typically longer than headers, contains readable text
              if (text.length > 20 && text.length < 2000 && 
                  !text.includes('Generated by') && 
                  !text.includes('Contains data') &&
                  !text.match(/^\d+\s*(day|hour|minute|second|week|month|year)/) &&
                  text.match(/[a-zA-Z]{5,}/)) { // At least 5 character words

                // Check if this looks like a post (not just metadata)
                if (!text.includes('Facebook') || text.length > 50) {
                  const children = $(div).children().length;
                  if (children < 20) { // Not a container with too many children
                    postCount++;
                    foundPostContent = true;
            ...
            if (!foundPostContent) {
              // Fallback: count table rows which might contain posts
