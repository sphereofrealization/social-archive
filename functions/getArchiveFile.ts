...
Deno.serve(async (req) => {
  try {
    ...
    // PHASE 2: Parse HTML and JSON files
    console.log("📄 Phase 2: Parsing HTML and JSON files...");

    // First extract profile name
    for (const [path, file] of Object.entries(zip.files)) {
    ...
    for (const [path, file] of Object.entries(zip.files)) {
      ...
      try {
        ...
        // POSTS - Facebook stores posts as JSON
        if (path.match(/your_posts.*\.json/i)) {
          try {
            const json = JSON.parse(content);
            if (Array.isArray(json)) {
              json.forEach(post => {
                const postData = post.data?.[0] || post;
                const postText = postData.post || postData.title || "";
                ...
                if (postText && postText.length > 5) {
                  ...
                  let photo_url = null;
                  if (post.attachments) {
                    const photoData = post.attachments[0]?.data;
                    if (photoData) {
                      const mediaPath = photoData[0]?.media?.uri;
                      if (mediaPath) {
                        const fullPath = path.split('/').slice(0, -1).join('/') + '/' + mediaPath;
                        if (data.photoFiles[fullPath]) {
                          photo_url = data.photoFiles[fullPath];
                        }
                      }
                    }
                  }
            ...
            console.error(`Failed to parse posts JSON ${path}:`, e.message);
          }
        }

        // COMMENTS - also stored as JSON
        if (path.match(/comments.*\.json/i)) {
          try {
            const json = JSON.parse(content);
            if (json.comments) {
              json.comments.forEach(comment => {
                const commentData = comment.data?.[0] || comment;
                const text = commentData.comment?.comment || "";
            ...
            console.error(`Failed to parse comments JSON ${path}:`, e.message);
          }
        }

        // FRIENDS
        if (path.match(/friends.*\.json/i)) {
          try {
            const json = JSON.parse(content);
        ...
        // GROUPS - parse from HTML
        if (path.match(/groups.*\.html/i)) {
          const bodyContent = getBodyContent(content);
          const items = bodyContent.match(/<a[^>]*>([^<]+)<\/a>/gi) || [];
          items.forEach(item => {
            const name = extractText(item);
            if (name.length > 3 && name.length < 150 && 
                !name.match(/^(Group|Privacy|Settings|Facebook)$/i) &&
